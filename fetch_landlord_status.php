...
// Check if the landlord ID is set
if (isset($_GET['id'])) {
    $landlord_id = $_GET['id'];

    // Prepare and bind
    $sql = "SELECT landlord_status FROM landlord_registration WHERE id = ?";
        ...
        if ($result->num_rows > 0) {
            ...
            switch ($status) {
                ...
                case 'rejected':
                    ...
                    $action = '<button onclick="deleteAccount(' . $landlord_id . ')">Exit</button>';
                    break;
                case 'pending':
                default:
            ...
            // If no landlord found, redirect to login form
    ...
    echo json_encode(['message' => 'No landlord ID provided.']);
}
